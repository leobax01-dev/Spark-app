...
// Premium agent, timed to each agent's own morning, no browser involved.
//
// Runs HOURLY (see vercel.json), not once a day at one fixed UTC time —
// Vercel Cron schedules are static and can't be "per agent," so instead
// every hourly invocation checks each agent's stored timezone and only
// does the real work for whoever's local clock currently reads
// TARGET_LOCAL_HOUR. An agent in Seattle and one in Miami both get their
// briefing at their own 7am, generated by two different hourly ticks of
// this same job, not simultaneously.
//
// The client-side engines (aggregateBusinessData/runAutopilotEngine/
// getMarketContext, computeSphereCandidates/generateSphereReactivation,
...
// ─── Per-agent timezone scheduling ─────────────────────────────────────────
// Vercel Cron schedules are static and can't be "per user," so this runs
// hourly instead of once a day, and each invocation only processes agents
// whose LOCAL time right now matches their morning hour. An agent in LA
...
function getLocalHour(timezone){
  try{
    const formatter = new Intl.DateTimeFormat("en-US", { timeZone: timezone||DEFAULT_TIMEZONE, hour:"numeric", hour12:false });
    const hourStr = formatter.format(new Date());
    return parseInt(hourStr, 10) % 24; // Intl can return "24" for midnight in some locales
  }catch(e){
    // Invalid/unrecognized timezone string — fall back rather than skip the agent entirely
    console.warn("Invalid timezone, using default:", timezone, e.message);
    const formatter = new Intl.DateTimeFormat("en-US", { timeZone: DEFAULT_TIMEZONE, hour:"numeric", hour12:false });
