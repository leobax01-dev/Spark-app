...
function featuresAt(event, bbox, ctx, buffer) {
  ...
  if (ctx.options.styles)
    queryParams.layers = ctx.options.styles.map((s) => s.id).filter((id) => ctx.map.getLayer(id) != null);
...
ModeInterface.prototype.newFeature = function(geojson) {
  ...
  if (type === geojsonTypes.POINT)
    return new point_default(this._ctx, geojson);
  if (type === geojsonTypes.LINE_STRING)
    return new line_string_default(this._ctx, geojson);
  if (type === geojsonTypes.POLYGON)
    return new polygon_default(this._ctx, geojson);
  return new multi_feature_default(this._ctx, geojson);
};
ModeInterface.prototype.isInstanceOf = function(type, feature) {
  if (type === geojsonTypes.POINT)
    return feature instanceof point_default;
  if (type === geojsonTypes.LINE_STRING)
    return feature instanceof line_string_default;
  if (type === geojsonTypes.POLYGON)
    return feature instanceof polygon_default;
  if (type === "MultiFeature")
    return feature instanceof multi_feature_default;
...
function events_default(ctx) {
  ...
  function actionable(actions) {
    ...
    if (changed)
      ctx.map.fire(events.ACTIONABLE, { actions: actionState });
  }
  const api = {
    start() {
...
SimpleSelect.onMouseMove = function(state, e) {
  ...
  if (isFeature2 && state.dragMoving)
    this.fireUpdate();
...
function api_default(ctx, api) {
  ...
  api.changeMode = function(mode, modeOptions = {}) {
    if (mode === modes.SIMPLE_SELECT && api.getMode() === modes.SIMPLE_SELECT) {
      if (string_sets_are_equal_default(modeOptions.featureIds || [], ctx.store.getSelectedIds()))
        return api;
